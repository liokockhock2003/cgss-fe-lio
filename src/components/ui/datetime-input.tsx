...
import { useRef, useState, useMemo, useEffect, useLayoutEffect, useCallback, RefObject } from 'react'
...
const DateTimeInput = forwardRef<HTMLInputElement, DateTimeInputProps>((options: DateTimeInputProps, ref) => {
  const { format: formatProp, value: _value, timezone, checker, ...rest } = options
  const value = useMemo(() => (_value ? new TZDate(_value, timezone) : undefined), [_value, timezone])
  const form = useFormContext()
  const formatStr = useMemo(() => formatProp || 'dd/MM/yyyy-hh:mm aa', [formatProp])
  ...
  return (
    <div
      ...
      <input
        ...
        className='flex-grow min-w-0 bg-transparent py-1 ps-1 pe-2 text-sm focus:outline-none disabled:cursor-not-allowed disabled:opacity-50'
        ...
        <div className='me-3'>
          ...
          : <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    Please enter a valid value. The input cannot be empty and must be within the range of years 1900 to
                    2100.
                  </p>
                </TooltipContent>
              </Tooltip>
...
function setSelection(ref: RefObject<HTMLInputElement | undefined>, segment?: Segment) {
