...
//
// export function PieScope1({ query }: { query: UseQueryResult<DashboardEmissionResponse> }) {
//   const [selected, setSelected] = useState<string[]>([])
//
//   useEffect(() => {
//     setSelected(query.data?.pieScope1.map((i) => i.type) ?? [])
//   }, [query.data?.pieScope1])
//
//   const _data = useMemo(
//     () => query.data?.pieScope1.filter((item) => selected.includes(item.type)) ?? [],
//     [selected, query.data?.pieScope1],
//   )
//
...
//     () => orderBy(query.data?.pieScope1, 'emissions', 'desc').map((g) => ({ value: g.type, label: g.type })),
//     [query.data?.pieScope1],
//   )
//
//   return (
...
//           Scope 1<span className='text-muted-foreground'>({ChemicalSymbol('TONCO2E')})</span>
//         </CardTitle>
//
...
//                 <span>{humanFormat(query.data?.pieScope1.find((_) => _.type === option.value)?.emissions)}</span>
//               </div>
//             )}
...
//                   {value?.length === 0 ?
//                     'Select types'
//                   : value?.length === query.data?.pieScope1.length ?
//                     'All types'
...
//               <Pie data={_data} innerRadius={60} strokeWidth={5} dataKey='emissions' nameKey='type'>
//                 <LabelList
//                   dataKey='type'
