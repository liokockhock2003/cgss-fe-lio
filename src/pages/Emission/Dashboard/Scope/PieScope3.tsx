...
//
// const chartConfig = {
//   bt: { label: 'Business Travel' },
//   ec: { label: 'Employee Commuting' },
//   upstream: { label: 'Upstream' },
//   downstream: { label: 'Downstream' },
// } satisfies ChartConfig
//
// export function PieScope3({ query }: { query: UseQueryResult<DashboardEmissionResponse> }) {
//   return null
//   const [selected, setSelected] = useState<string[]>([])
//
//   useEffect(() => {
//     setSelected(query.data?.pieScope3.map((i) => i.type) ?? [])
//   }, [query.data?.pieScope3])
//
//   const _data = useMemo(() => {
//     return query.data?.pieScope3.filter((item) => selected.includes(item.type))
//   }, [selected, query.data?.pieScope3])
//
//   const total = useMemo(() => humanFormat(sumBy(_data, 'emissions')), [_data])
//   const isEmpty = useMemo(() => _data.every((i) => i.emissions === 0), [_data])
//
//   const _options = useMemo(
//     () => orderBy(query.data?.pieScope3, 'emissions', 'desc').map((g) => ({ value: g.type, label: g.type })),
//     [query.data?.pieScope3],
//   )
//
//   return (
...
//           Scope 3<span className='text-muted-foreground'>({ChemicalSymbol('TONCO2E')})</span>
//         </CardTitle>
//
...
//                 <span>{humanFormat(query.data?.pieScope3.find((_) => _.type === option.value)?.emissions)}</span>
//               </div>
//             )}
...
//                   {value?.length === 0 ?
//                     'Select types'
//                   : value?.length === query.data?.pieScope3.length ?
//                     'All types'
