...
import { emissionProductionQueryFilter } from './helper.tsx'

export type DialogActivityPayload = {
  row: TEmissionProduction['main']['base']
  columnDef: ColumnDef<TEmissionProduction['main']['base']>
}

DialogActivity.displayName = 'EmissionProductionDialogActivity'
export function DialogActivity({
  ...
  const canCreate = useCan({ permission: 'emission-production.create', groupById: row.groupById })

  const query = useQuery(
    EmissionProductionApi.activities.list()({
      params: emissionProductionQueryFilter({ year, month, id: row.id }),
    }),
  )
