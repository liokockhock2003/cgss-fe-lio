...
ActivityForm.displayName = 'EmissionProductionActivityForm'
export function ActivityForm({
  ...
  const mutation = useMutation(
    EmissionProductionApi.activities.mutationOption({ type, onSuccess: handleSecondaryAction }),
  )
  ...
  const onSubmit = (values) => {
    ...
    mutation.mutate(
      type === 'create' ? { ...values, date, emissionProductionId: row.id } : { id: data.id, ...values, date },
    )
  }
          ...
          <div className='grid grid-cols-2 gap-2'>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    Input<span className='ml-2 text-muted-foreground'>(ton)</span>
                  </FormLabel>
                  <FormControl>
