...
EmissionProductionHeader.displayName = 'EmissionProductionHeader'
export function EmissionProductionHeader() {
  const { table, query } = useEmissionProduction()
  const { popoverStates, handleOpenChangeWith } = usePopoverStates<{ create: boolean }>({ create: undefined })
  const canCreate = useCan({ permission: 'emission-production.create' })
  ...
  return (
    <>
      ...
      {popoverStates.create ?
        <DialogCategory type='create' onClose={() => handleOpenChangeWith('create', undefined)} />
      : null}
    </>
  )
}
