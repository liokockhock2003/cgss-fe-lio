...
const columnHelper = createColumnHelper<TEmissionProduction['main']['base']>()
...
const generateActivityColDef = (listOfYearsAndMonths: GenerateListOfYearsAndMonths) => {
  ...
  return listOfYearsAndMonths.map(([yearStr, path]) => {
    return columnHelper.group({
      id: yearStr,
      header: ({ column, table }) => <CellTotalHeader column={column} table={table} subtitle='ton' />,
      columns: [
        // total view
...
): ColumnDef<TEmissionProduction['main']['base']>[] => {
  return [
