...
import { DialogCategory } from './DialogCategory.tsx'
import { EmissionProductionApi, type TEmissionProduction } from './api.ts'
import { useEmissionProduction } from './context.tsx'

RowActions.displayName = 'EmissionProductionRowActions'
export function RowActions({ row }: { row: Row<TEmissionProduction['main']['base']> }) {
  const { queryKey } = useEmissionProduction()
  const canUpdate = useCan({ permission: 'emission-production.update', groupById: row.original.groupById })
  const canDelete = useCan({ permission: 'emission-production.delete', groupById: row.original.groupById })
  const hasAnyPermission = canUpdate || canDelete

  const { popoverStates, handleOpenChangeWith } = usePopoverStates<
    Partial<{
      deleteId: number
      edit: TEmissionProduction['main']['base']
