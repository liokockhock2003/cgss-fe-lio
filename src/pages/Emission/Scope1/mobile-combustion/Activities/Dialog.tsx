...
export type DialogActivityPayload = {
  row: TMobileCombustion['main']['base']
  columnDef: ColumnDef<TMobileCombustion['main']['base']>
}
...
export function DialogActivity({
  ...
  const canCreate = useCan({ permission: 'scope1.mobile-combustion.create', groupById: row.groupById })

  const query = useQuery(
    MobileCombustionApi.activities.optimize.useQuery({ payload: { mobileCombustionId: row.id, year, month } }),
  )
      ...
      <DialogContent className='max-w-lg md:max-w-3xl flex flex-col max-h-[95vh]' aria-describedby={undefined}>
        <DialogHeader>
          ...
          <div className='flex items-center gap-0.5 text-sm'>
            <p>{row.mobileRegistry.identity_no}</p>
            <p>({row.mobileRegistry.model})</p>
            {row.mobileRegistry.hasLitreId ?
              <CommonIcon type='liquid' />
            : null}
          </div>
        </DialogHeader>
