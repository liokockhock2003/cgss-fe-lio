...
import type { DialogActivityPayload } from '../Activities/Dialog.tsx'
import { MobileCombustionApi, type TMobileCombustion } from '../api.ts'

ActivityForm.displayName = 'MobileCombustion ActivityForm'
export function ActivityForm({
  ...
  const initialValue = useMemo(
    () => ({ input: undefined, desc: '', date: minDate, type: 'distance' as 'distance' | 'litre' }),
    [],
  )
      ...
      type === 'edit' ?
        {
          ...
          input: data.input,
          desc: data.desc,
          type: row.mobileRegistry.hasLitreId ? data.type : 'distance',
        }
      : initialValue,
  ...
  const mutation = useMutation(
    q$.scope._1.mobileCombustion.activities.mutationOption({
      type,
      onSuccess: () => {
        q$.invalidateQuery(MobileCombustionApi.activities.optimize.qk({ mobileCombustionId: row.id, year, month }))
  ...
  const onSubmit = (values) => {
    ...
    mutation.mutate(
      type === 'create' ? { ...values, date, mobileCombustionId: row.id } : { id: data.id, ...values, date },
    )
  }
