...
import { DialogActivityPayload } from '../Activities/Dialog.tsx'
import { MobileCombustionApi, TMobileCombustion } from '../api.ts'

ActivityList.displayName = 'MobileCombustion ActivityList'
export function ActivityList({
  data,
  setCurrentEdit,
  rowColumnInfo,
}: {
  data: TMobileCombustion['activities']
  ...
  const canUpdate = useCan({ permission: 'scope1.mobile-combustion.update', groupById: row.groupById })
  const canDelete = useCan({ permission: 'scope1.mobile-combustion.delete', groupById: row.groupById })

  const deleteMutation = useMutation(
    MobileCombustionApi.activities.mutationOption({
      type: 'delete',
      onSuccess: async () => {
        handleOpenChangeWith('isDeleting', false)
        await queryClient.invalidateQueries({
          queryKey: MobileCombustionApi.activities.optimize.qk({ mobileCombustionId: row.id, year, month }),
        })
