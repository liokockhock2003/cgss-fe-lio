...
import { MobileCombustionContext } from './context.tsx'

MobileCombustionProvider.displayName = 'MobileCombustion Provider'
export function MobileCombustionProvider({ children }) {
  ...
  const queryKey = [MobileCombustionApi.main.uniqueKey, 'optimize', { ...tableQueries, from, to, groupByIds }] as const

  const query = useQuery({
    placeholderData: keepPreviousData,
    queryKey,
    queryFn: async (): Promise<TMobileCombustion['main']['response']> => {
      const params = { groupByIds, ...tableQueries, ...isDebug(), from, to }
      return await axios.get(`/${MobileCombustionApi.main.uniqueKey}/optimize`, { params }).then((i) => i.data)
    },
  })
  ...
  const table = useReactTable({
    ...
    initialState: { columnPinning: { left: ['mobileRegistry.identity_no'], right: ['actions'] } },
    onSortingChange,
    onPaginationChange,
  ...
  return <MobileCombustionContext.Provider value={value}>{children(value)}</MobileCombustionContext.Provider>
}
