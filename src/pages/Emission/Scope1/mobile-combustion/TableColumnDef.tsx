...
const columnHelper = createColumnHelper<TMobileCombustion['main']['base']>()
...
): ColumnDef<TMobileCombustion['main']['base']>[] => {
  return [
    columnHelper.accessor('mobileRegistry.identity_no', {
      id: 'mobileRegistry.identity_no',
      enableSorting: true,
      enablePinning: true,
      ...
      cell: ({ getValue, row }) => {
        return (
          <div className='whitespace-nowrap p-4 flex justify-between items-center'>
            {getValue()}
            <span className='opacity-65'> ({row.original.mobileRegistry.model})</span>
          </div>
        )
      },
      ...
      meta: {
        displayAs: 'Name',
        cellClass: ({ row }) =>
          [row.original.status, row.original.mobileRegistry.status].some((i) => i === 'inactive') ? inActiveClass : '',
        headerClass: () => 'px-4',
      },
    }),
