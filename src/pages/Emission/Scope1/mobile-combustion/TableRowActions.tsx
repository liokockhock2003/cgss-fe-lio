...
RowActions.displayName = 'MobileCombustion RowActions'
export function RowActions({ row }: { row: Row<TMobileCombustion['main']['base'] & { groupById: number }> }) {
  const { queryKey } = useMobileCombustion()
  const canUpdate = useCan({ permission: 'scope1.mobile-combustion.update', groupById: row.original.groupById })
  const canDelete = useCan({ permission: 'scope1.mobile-combustion.delete', groupById: row.original.groupById })
  const hasAnyPermission = canUpdate || canDelete

  const { popoverStates, handleOpenChangeWith } = usePopoverStates<
    Partial<{
      deleteId: number
      edit: TMobileCombustion['main']['base']
