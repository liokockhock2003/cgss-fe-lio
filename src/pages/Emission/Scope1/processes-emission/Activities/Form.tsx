...
export const processEmissionQueryFilter = ({ year, month, id }) => {
  return {
    filter: {
      order: ['date'],
      where: {
        processEmissionId: id,
...
ActivityForm.displayName = 'ProcessEmission ActivityForm'
export function ActivityForm({
  ...
  const mutation = useMutation(ProcessEmissionApi.activities.mutationOption({ type, onSuccess: handleSecondaryAction }))

  const onSubmit = (values) => {
    const date = format(new Date(values.date), "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'")

    mutation.mutate(
      type === 'create' ? { ...values, date, processEmissionId: row.id } : { id: data.id, ...values, date },
    )
  }
