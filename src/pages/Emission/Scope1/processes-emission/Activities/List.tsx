...
ActivityList.displayName = 'ProcessEmission ActivityList'
export function ActivityList({
  data,
  setCurrentEdit,
  rowColumnInfo,
}: {
  data: TProcessEmission['activities']
  ...
  const canUpdate = useCan({ permission: 'scope1.processes-emission.update', groupById: row.groupById })
  const canDelete = useCan({ permission: 'scope1.processes-emission.delete', groupById: row.groupById })

  const deleteMutation = useMutation(
    ProcessEmissionApi.activities.mutationOption({
      type: 'delete',
      onSuccess: async () => {
        handleOpenChangeWith('isDeleting', false)
        await queryClient.invalidateQueries({
          queryKey: ProcessEmissionApi.activities.optimize.qk({ processEmissionId: row.id, year, month }),
        })
