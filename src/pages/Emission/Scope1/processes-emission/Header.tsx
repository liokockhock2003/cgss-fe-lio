...
ProcessEmissionHeader.displayName = 'ProcessEmission Header'
export function ProcessEmissionHeader() {
  const { table, query } = useProcessEmission()
  const { popoverStates, handleOpenChangeWith } = usePopoverStates<{ create: boolean }>({ create: undefined })
  const canCreate = useCan({ permission: 'scope1.processes-emission.create' })
