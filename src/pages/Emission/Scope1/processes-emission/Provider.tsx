...
import { ProcessEmissionContext } from './context.tsx'

ProcessEmissionProvider.displayName = 'ProcessEmission Provider'
export function ProcessEmissionProvider({ children }) {
  ...
  const queryKey = [ProcessEmissionApi.main.uniqueKey, 'optimize', { ...tableQueries, from, to, groupByIds }] as const

  const query = useQuery({
    placeholderData: keepPreviousData,
    queryKey,
    queryFn: async (): Promise<TProcessEmission['main']['response']> => {
      const params = { groupByIds, ...tableQueries, ...isDebug(), from, to }
      return await axios.get(`/${ProcessEmissionApi.main.uniqueKey}/optimize`, { params }).then((i) => i.data)
    },
  })
  ...
  return <ProcessEmissionContext.Provider value={value}>{children(value)}</ProcessEmissionContext.Provider>
}
