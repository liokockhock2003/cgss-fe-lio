...
import { StateBadge } from '@/components/common-icon'
...
export type DialogActivityPayload = {
  row: TStationaryCombustion['main']['base']
  columnDef: ColumnDef<TStationaryCombustion['main']['base']>
}
...
export function DialogActivity({
  ...
  const query = useQuery(
    StationaryCombustionApi.activities.optimize.useQuery({
      payload: { stationaryCombustionId: row.id, year, month },
    }),
  )
  const canCreate = useCan({ permission: 'scope1.stationary-combustion.create', groupById: row.groupById })
      ...
      <DialogContent className='max-w-lg md:max-w-3xl flex flex-col max-h-[95vh]' aria-describedby={undefined}>
        <DialogHeader>
          ...
          <div className='flex items-center gap-0.5 text-sm'>
            <p>{row.type.fuel_types}</p>
            <ChevronRight className='h-4 w-4' />
            <p className='mr-2'>{row.type.name}</p>
            <StateBadge type={row.type.state as never} unit={row.type.unit} />
          </div>
        </DialogHeader>
