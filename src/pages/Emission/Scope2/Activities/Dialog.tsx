...
export type DialogActivityPayload = {
  row: TScope2['main']['base']
  columnDef: ColumnDef<TScope2['main']['base']>
}

DialogActivity.displayName = 'Scope2 DialogActivity'
export function DialogActivity({
  ...
  const query = useQuery(Scope2Api.activities.optimize.useQuery({ payload: { scope2Id: row.id, year, month } }))
  const canCreate = useCan({ permission: 'scope2.scope2.create', groupById: row.groupById })
      ...
      <DialogContent className='max-w-lg md:max-w-3xl flex flex-col max-h-[95vh]' aria-describedby={undefined}>
        <DialogHeader>
          ...
          <div className='flex items-center gap-0.5 text-sm'>
            <p>{row.category || '-'}</p>
            <ChevronRight className='h-4 w-4' />
            <CommonIcon type={row.type} className='size-5' />
            <p>{row.type}</p>
            {row.type === 'electric' ?
              <>
                <ChevronRight className='h-4 w-4' />
