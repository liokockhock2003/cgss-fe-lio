...
ActivityForm.displayName = 'Scope2 ActivityForm'
export function ActivityForm({
  ...
  const mutation = useMutation(
    Scope2Api.activities.mutationOption({
      type,
      onSuccess: () => {
        q$.invalidateQuery(Scope2Api.activities.optimize.qk({ scope2Id: row.id, year, month }))
  ...
  const onSubmit = (values) => {
    ...
    mutation.mutate(type === 'create' ? { ...values, date, scope2Id: row.id } : { id: data.id, ...values, date })
  }
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    Input
                    {row.type === 'electric' ?
                      <span className='ml-2 text-muted-foreground'>(kWh)</span>
                    : null}
                  </FormLabel>
                  <FormControl>
