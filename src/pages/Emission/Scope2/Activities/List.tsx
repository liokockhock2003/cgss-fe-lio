...
ActivityList.displayName = 'Scope2 ActivityList'
export function ActivityList({
  data,
  setCurrentEdit,
  rowColumnInfo,
}: {
  data: TScope2['activities']
  ...
  const canUpdate = useCan({ permission: 'scope2.scope2.update', groupById: row.groupById })
  const canDelete = useCan({ permission: 'scope2.scope2.delete', groupById: row.groupById })

  const deleteMutation = useMutation(
    q$.scope._2.activities.mutationOption({
      type: 'delete',
      onSuccess: async () => {
        handleOpenChangeWith('isDeleting', false)
        await queryClient.invalidateQueries({
          queryKey: q$.scope._2.activities.optimize.qk({ scope2Id: row.id, year, month }),
        })
