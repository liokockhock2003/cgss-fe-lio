...
import { CellActivities } from '@/components/tanstack-table/cell/cell-activities.tsx'
...
import { CellActivityHeader } from './ActivityHeader.tsx'
import { RowActions } from './TableRowActions.tsx'
import type { TEmployeeCommuting } from './api.ts'

const columnHelper = createColumnHelper<TEmployeeCommuting['main']['base']>()
...
): ColumnDef<TEmployeeCommuting['main']['base']>[] => {
  return [
    columnHelper.accessor('employeeRegistry.name', {
      id: 'employeeRegistry.name',
      enableSorting: true,
      enablePinning: true,
      ...
      cell: ({ getValue, row }) => (
        <div className='whitespace-nowrap p-4'>
          {getValue()}
          {row.original.employeeRegistry.staffId ?
            <span className='opacity-65'> ({row.original.employeeRegistry.staffId})</span>
          : null}
        </div>
      ),
      ...
      meta: {
        ...
        cellClass: ({ row }) =>
          [row.original.status, row.original.employeeRegistry.status].some((i) => i === 'inactive') ? inActiveClass : (
            ''
          ),
        headerClass: () => 'px-4',
      },
