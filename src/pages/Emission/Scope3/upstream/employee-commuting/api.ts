...
type EmployeeCommutingMain = {
  ...
  response: {
    rowCount: number
    rows: EmployeeCommutingMain['base'][]
  }
}

const uniqueKey = 'emission-scope3-employee-commuting'

const main = {
  uniqueKey,
  ...QueryFactory<EmployeeCommutingMain['response']>(uniqueKey),
  optimized: {
    qk: () => [...main.qk(), 'optimize'],
    useQuery<T>(_action?: {
...
export const EmployeeCommutingApi = { main, activities }
export type TEmployeeCommuting = { main: EmployeeCommutingMain; activities: EmployeeCommutingActivity }
