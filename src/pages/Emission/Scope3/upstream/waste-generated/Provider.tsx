...
import { WasteGeneratedContext } from './context.tsx'

WasterGeneratedProvider.displayName = 'WasteGenerated Provider'
export function WasterGeneratedProvider({ children }) {
  ...
  const queryKey = [WasteGeneratedApi.main.uniqueKey, 'optimize', { ...tableQueries, from, to, groupByIds }] as const

  const query = useQuery({
    placeholderData: keepPreviousData,
    queryKey,
    queryFn: async (): Promise<TWasteGenerated['main']['response']> => {
      const params = { groupByIds, ...tableQueries, ...isDebug(), from, to }
      return await axios.get(`/${WasteGeneratedApi.main.uniqueKey}/optimize`, { params }).then((i) => i.data)
    },
  })
  ...
  return <WasteGeneratedContext.Provider value={value}>{children(value)}</WasteGeneratedContext.Provider>
}
