...
RowActions.displayName = 'WasteGenerated RowActions'
export function RowActions({ row }: { row: Row<TWasteGenerated['main']['base']> }) {
  const { queryKey } = useWasteGenerated()
  const canUpdate = useCan({ permission: 'scope3.waste-generated.update', groupById: row.original.groupById })
  const canDelete = useCan({ permission: 'scope3.waste-generated.delete', groupById: row.original.groupById })
  const hasAnyPermission = canUpdate || canDelete

  const { popoverStates, handleOpenChangeWith } = usePopoverStates<
    Partial<{
      deleteId: number
      edit: TWasteGenerated['main']['base']
