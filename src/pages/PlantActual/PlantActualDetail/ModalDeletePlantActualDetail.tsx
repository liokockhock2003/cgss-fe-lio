...
export const ModalDeletePlantActualDetail = ({ data, onApply }) => {
  ...
  const onDelete = () => {
    ...
    axios
      .delete(`plant-actual-details/${data.id}`, { signal: controller.signal })
      .then(() => {
        onApply(data.id)
    ...
    <>
      ...
      <CModal scrollable visible={modalVisible} onClose={() => setModalVisible(false)}>
        <CModalHeader className='text-white text-center' style={{ backgroundColor: '#044A89' }}>
          <CModalTitle>Delete Actual Plant Detail</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CRow>
            <CCol>
              <h4 className='tbl' style={{ fontSize: '16px', fontWeight: '400' }}>
                Are you sure to delete this actual plant detail
              </h4>
            </CCol>
          </CRow>
        </CModalBody>
...
ModalDeletePlantActualDetail.propTypes = {
