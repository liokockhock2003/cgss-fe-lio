...
export const ModalDeleteState = ({ label, data, onApply }) => {
  ...
  const onDelete = () => {
    ...
    axios
      .delete(`states/${data.id}`, { signal: controller.signal })
      .then(() => {
        onApply(data.id)
    ...
    <>
      ...
      <CModal scrollable visible={modalVisible} onClose={() => setModalVisible(false)}>
        <CModalHeader className='text-white text-center' style={{ backgroundColor: '#044A89' }}>
          <CModalTitle>Delete State</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CRow>
            <CCol>
